...
이 기사에는 비전 데이터를 예시로 수집된 데이터에서 데이터셋 생성까지 과정을 자동화하는 샘플 아키텍처 제시하고, 비디오 데이터에서 이미지 데이터를 샘플링하는 간단한 파이프라인 예제를 추가하였습니다.

<!--more-->

비전 데이터셋은 데이터셋에 추가 할 이미지 샘플링을 어떻게 하느냐에 따라 딥러닝 프로세스 전체에 막대한 영향을 끼칩니다. 가장 좋은 방법은 SME (Subject Matter Expert)가 직접 이미지 파일을 캡처해서 수집하는 것이지만, 프로젝트에 따라 매우 힘들거나 너무 비효율적 일 때가 많습니다. 이 기사에는 모델이 사용될 환경에서 수집된 비디오 파일에서 라벨링이 필요한 이미지 파일을 추출하는 샘플링 방법을 사용하여 데이터셋을 생성하는 것으로 가정 하였습니다.

AI 선도 업체들은 이미 *[반지도 학습 (Semi-supervised Learning)](https://en.wikipedia.org/wiki/Semi-supervised_learning), [능동적 학습 (Active Learning)](https://blogs.nvidia.co.kr/2020/01/29/what-is-active-learning/), [자동 라벨링 (Auto Labeling)](https://docs.aws.amazon.com/sagemaker/latest/dg/sms-automated-labeling.html)*등의 현대적인 기법들을 적용하여 데이터셋 생성에 드는 비용을 최소화 하고 있습니다. 이 기사는 최신 라벨링 기법들 적용에 앞서 기반이 되는 아키텍처를 어떻게 해야 할지 갈피를 잡지 못하는 분들을 위해 작성하였습니다.
...
앞서 설명한 GCP에 가장 기본적인 형태로 데이터셋 아키텍처를 구성해 보았습니다. 이렇게 아키텍처를 구성하면 크게 두가지 문제에 직면하게 됩니다.

### 클라우드 비용 문제

비디오 데이터는 저장 장치 용량을 많이 차지하므로 수집되는 데이터에 비례하여 클라우드 사용료가 큰 폭으로 증가하게 됩니다. 이런 문제를 해결하기 위해 아래와 같이 추출한 이미지 파일만 클라우드에 업로드하는 하이브리드 클라우드 방식으로 아키텍처링을 할 수 있습니다.
...
자율주행 데이터셋을 수집하는 시나리오를 생각해보면, 1초마다 프레임을 추출하는 것은 정지된 자동차의 영상에는 유사한 이미지를 여러장 추출할 가능성이 매우 큽니다. _능동적 학습_ 알고리즘을 적용하기 이전에 *이미지 유사도*를 검사하여 유사도가 높은 이미지는 샘플링 하지 않는 간단한 알고리즘으로도 샘플링 이미지 개수를 크게 줄일수 있을 것 입니다.
